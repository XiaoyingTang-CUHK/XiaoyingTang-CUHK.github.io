...
//var  show="1";
	...
	zyMedia.MediaPlayer.prototype = {
		...
		setPlayerSize: function(width, height) {
			...
			//$(".live_head").css("height",t.height+75);
            //$(".live_body").css("paddingTop",t.height+75);
		},
		...
		buildFullscreen: function() {
			...
			t.fullscreenBtn.addEventListener('tap', function() {
		...
		buildDec: function() {
			...
			all.className = 'dec_all';
			all.style.display = 'none';
			t.container.appendChild(all);

$(".dec_all").click(function(){
  console.log("1");
$(".dec_all").hide();
 $(".zy_controls").show();
        $(".zy_title").show();
...
$(".dec_all").show();
		...
		init: function() {
			...
			if (t.isVideo) {
				if (zyMedia.features.hasTouch) {
					t.media.addEventListener('tap', function() {
						// Toggle controls
						if (t.isControlsVisible) {
							t.hideControls()
						} else {
